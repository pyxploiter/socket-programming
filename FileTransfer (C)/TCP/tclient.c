...
int main(){
    int count = 1;
    char filename[50];
    printf("Enter the audio file name(e.g. sending_file.mp3): ");
    scanf("%s", filename);
	
    FILE *sending_file;
    if((sending_file = fopen(filename,"rb")) == NULL){
	exit(0);
    }
    fseek(sending_file, 0, SEEK_END);
    int size = ftell(sending_file);
    fseek(sending_file, 0, SEEK_SET);
    ...
    *((int*)chr_size) = size;   // typecasting int to char[]
    ...
    while(count < (no_packets+1)){
        fread(packet, sizeof(char),sizeof(packet),sending_file);
    ...
    fread(packet, sizeof(char),(size-(no_packets*PACKET_SIZE)),sending_file);
